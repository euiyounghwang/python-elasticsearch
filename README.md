...
`'/cluster/sharding_predict'` with POST method against data size (<i>You will want to limit your maximum shard size to 30-80 GB if running a recent version of Elasticsearch. or instance, let’s assume you rotate indices monthly and expect around 600 GB of data per month. In this example, you would allocate 8 to 20 shards.</i>)
